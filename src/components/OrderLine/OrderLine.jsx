...
function OrderLine({ data }) {
    ...
    return (
        ...
        <div className="dashboard__ordersSection__table__item" >
            ...
            <div>{(+data.buy_price).toFixed(4)}</div>
            <div>{(+data.sell_base_qty).toFixed(4) }</div>
            <div>{(+data.sell_quote_qty).toFixed(4)}</div>
            <div>{(+data.sell_price).toFixed(4)}</div>
