...
import {menusAuth, menusNonAuth} from '../../pages/menu';
...
import {auth, logOut} from "../../helpers/firebaseConfigAndControls";
...
const Header = () => {
    ...
    const {user, setUser, dictionary, dictionaries, defaultDictionary} = useContext(AppContext);
    //#endregion
    ...
    //#region Set menu content
    const [menu, setMenu] = useState(menusNonAuth);

    useEffect(() => {
        if (user?.idPost) {
            setMenu(menusAuth);
        } else {
            setMenu(menusNonAuth);
        }
    }, [user]);
    //#endregion
        ...
        <header id="header_main" className={`header ${scroll ? 'is-fixed' : ''}`}>
            <div className="container-fluid">
                ...
                <div className="col-12">
                            ...
                            <nav id="main-nav" className={`main-nav ${menuActive ? 'active' : ''}`}>
                                <ul id="menu-primary-menu" className="menu">
                                {
                                    menu.map((data,idx) => (
                                        ...
                                        >
                                            <Link to={data.links} onClick={handleMenuActive}>{data.name}</Link>
                                            {
                                                data.namesub &&
                    ...
                    {/*#region Old right part of header*/}
                    ...
                    {/*                    <Dropdown.Item href="#"*/}
                    {/*                                   onClick={*/}
                    {/*                                       user?.idPost ? (event) => logOut(event, setUser, navigate, '/') : () => {}*/}
                    {/*                                   }*/}
                    {/*                    >*/}
                    {/*                        <Link className="dropdown-item text-danger"*/}
                    {/*                            // to={auth?.currentUser ? '/' : '/login'}*/}
                    {/*                              to={'/login'}*/}
                    {/*                        ><i*/}
                    {/*                            className="bx bx-power-off font-size-16 align-middle me-1 text-danger"*/}
                    {/*                        ></i>*/}
                    {/*                            <span>*/}
                    {/*                    {user?.idPost ? getTranslation('Logout', dictionary, defaultDictionary) : getTranslation('Login', dictionary, defaultDictionary)}*/}
                    {/*                </span>*/}
                    {/*                        </Link>*/}
                    {/*                    </Dropdown.Item>*/}
